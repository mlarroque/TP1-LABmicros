...
enum timers{INACTIVITY, DISPLAY}; //Timers utilizados
enum states{WAITING_ID, RECEIVING_ID, RECEIVING_PIN, ACCESS_GRANTED, ADMIN_MODE, ADDING_USER,
			REMOVING_USER, CHANGING_PIN}; //Estados posibles de la FSM
enum events{INPUT_EV, TIMER_EV, KEYCARD_EV};


//UserData contiene toda la informacion necesaria que necesita una rutina para manejar
//un evento que se saco de la cola.
typedef struct{
   char recieved_ID[ID_LENGTH]; //String con el ID recibido del usuario.
   char recieved_PIN[PIN_LENGTH]; //String con el PIN recibido del usuario.
